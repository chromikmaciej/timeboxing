...
describe('<Clock />', () => {
    ...
    describe('when given minutes and seconds (DOM)', () => {
        beforeEach(() => {
            ...
            ReactDOM.render(
                <Clock minutes={10} seconds={20} miliseconds={10} />, root
            )
        });
    ...
    describe('when given minutes and seconds (TestRenderer)', () => {
        beforeEach(() => {
            clockRenderer = renderer.create(
                <Clock minutes={10} seconds={20} miliseconds={10} />
            )
        });
        it('renders properly', () => {
            //expect(clockRenderer.toTree()).toEqual();
    ...
    it('renders properly when given minutes and seconds', () => {
        expect(<Clock minutes={10} seconds={20} miliseconds={10} />).toEqual(<Clock minutes={10} seconds={20} miliseconds={10} />)
    });
});